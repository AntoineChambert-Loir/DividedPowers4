...
noncomputable def dpow (I : Ideal A) : ℕ → A → A := fun m x =>
  if h : x ∈ I then Ring.inverse (m.factorial : A) * x ^ m else 0
#align divided_powers.of_invertible_factorial.dpow DividedPowers.OfInvertibleFactorial.dpow

theorem dpow_dif_pos {I : Ideal A} (m : ℕ) {x : A} (hx : x ∈ I) :
    dpow I m x = Ring.inverse (m.factorial : A) * x ^ m := by simp only [dpow] <;> rw [dif_pos hx]
#align divided_powers.of_invertible_factorial.dpow_dif_pos DividedPowers.OfInvertibleFactorial.dpow_dif_pos

theorem dpow_dif_neg {I : Ideal A} {m : ℕ} {x : A} (hx : x ∉ I) : dpow I m x = 0 := by
  simp only [dpow] <;> rw [dif_neg hx]
#align divided_powers.of_invertible_factorial.dpow_dif_neg DividedPowers.OfInvertibleFactorial.dpow_dif_neg
...
theorem dpow_one {x : A} (hx : x ∈ I) : dpow I 1 x = x := by
  rw [dpow_dif_pos 1 hx, pow_one, Nat.factorial_one, Nat.cast_one, Ring.inverse_one, one_mul]
...
theorem dpow_add_dif_pos {n : ℕ} (hn_fac : IsUnit ((n - 1).factorial : A)) {m : ℕ} (hmn : m < n)
  ...
  have h_ch :
    ...
    rw [Ring.eq_mul_inverse_iff_mul_eq _ _ _
      ...
      Ring.eq_mul_inverse_iff_mul_eq _ _ _ (factorial_isUnit hn_fac (lt_of_le_of_lt hk hmn))]
    nth_rw 1 [hadd]
    nth_rw 3 [hadd]
    rw [← Nat.cast_mul, ← Nat.cast_mul, Nat.add_choose_mul_factorial_mul_factorial]
  rw [dif_pos hx, dif_pos hy, h_ch, ← mul_assoc, ← mul_assoc,
...
theorem dpow_smul (m : ℕ) {a x : A} (hx : x ∈ I) : dpow I m (a * x) = a ^ m * dpow I m x := by
  rw [dpow_dif_pos m (Ideal.mul_mem_left I _ hx), dpow_dif_pos m hx, mul_pow, ← mul_assoc,
    mul_comm _ (a ^ m), mul_assoc]
...
theorem dpow_mul_dif_pos {n : ℕ} (hn_fac : IsUnit ((n - 1).factorial : A)) {m k : ℕ}
  ...
  have h_fac :
    Ring.inverse (m.factorial : A) * Ring.inverse k.factorial =
      ↑((m + k).choose m) * Ring.inverse (m + k).factorial :=
    by
    rw [Ring.eq_mul_inverse_iff_mul_eq _ _ _ (factorial_isUnit hn_fac hkm), mul_assoc,
    ...
    rw [← Nat.add_choose_mul_factorial_mul_factorial, mul_comm, mul_comm _ m.factorial,
      Nat.choose_symm_add]
  rw [dpow_dif_pos _ hx, dpow_dif_pos _ hx, dpow_dif_pos _ hx, mul_assoc, ← mul_assoc (x ^ m),
    mul_comm (x ^ m), mul_assoc _ (x ^ m), ← pow_add, ← mul_assoc, ← mul_assoc, h_fac]
...
theorem dpow_mul {n : ℕ} (hn_fac : IsUnit ((n - 1).factorial : A)) (hnI : I ^ n = 0) (m k : ℕ)
  ...
  · have hxmk : x ^ (m + k) = 0 := Ideal.mem_pow_eq_zero n (m + k) hnI (not_lt.mp hkm) hx
    rw [dpow_dif_pos m hx, dpow_dif_pos k hx, dpow_dif_pos (m + k) hx, mul_assoc, ←
      mul_assoc (x ^ m), mul_comm (x ^ m), mul_assoc _ (x ^ m), ← pow_add, hxmk,
...
theorem dpow_comp_dif_pos {n : ℕ} (hn_fac : IsUnit ((n - 1).factorial : A)) {m k : ℕ} (hk : k ≠ 0)
  ...
  · have hkn : k < n := lt_of_le_of_lt (Nat.le_mul_of_pos_left (Nat.pos_of_ne_zero hm0)) hkm
    ...
    have h_fac :
      Ring.inverse (m.factorial : A) * Ring.inverse k.factorial ^ m =
      ...
      rw [← mchoose_lemma _ hk, Nat.cast_mul, Nat.cast_mul, Nat.cast_pow, mul_comm ↑m.factorial,
        mul_assoc]
...
theorem dpow_comp {n : ℕ} (hn_fac : IsUnit ((n - 1).factorial : A)) (hnI : I ^ n = 0) (m : ℕ)
  ...
  · have hxmk : x ^ (m * k) = 0 := Ideal.mem_pow_eq_zero n (m * k) hnI (not_lt.mp hmk) hx
    rw [dpow_dif_pos _ (dpow_mem hk hx), dpow_dif_pos _ hx, dpow_dif_pos _ hx, mul_pow, ← pow_mul, ←
      mul_assoc, mul_comm k, hxmk, MulZeroClass.mul_zero, MulZeroClass.mul_zero,
...
noncomputable def dividedPowers {n : ℕ} (hn_fac : IsUnit ((n - 1).factorial : A))
    ...
    where
  dpow := dpow I
  dpow_null n x hx := dpow_null hx
  dpow_zero x hx := dpow_zero hx
  dpow_one x hx := dpow_one hx
  dpow_mem n hn x hx := dpow_mem hn hx
  dpow_add m x y hx hy := dpow_add hn_fac hnI m hx hy
  dpow_smul m a x hx := dpow_smul m hx
  dpow_mul m k x hx := dpow_mul hn_fac hnI m k hx
  dpow_comp m k hk x hx := dpow_comp hn_fac hnI m hk hx
...
theorem dpow_of_two_le {n : ℕ} (hn : 2 ≤ n) (a : A) : (dividedPowers hI2) n a = 0 :=
  by
  dsimp [DividedPowers, of_invertible_factorial.divided_powers, of_invertible_factorial.dpow]
  rw [coe_to_fun_apply]
  simp only
  split_ifs with ha; convert MulZeroClass.mul_zero _
...
theorem dpow_def (n : ℕ) {x : R} (hx : x ∈ I) : dpow I n x = Ring.inverse n.factorial * x ^ n := by
  simp only [dpow] <;> rw [of_invertible_factorial.dpow_dif_pos _ hx]
#align divided_powers.rat_algebra.dpow_def DividedPowers.RatAlgebra.dpow_def
...
noncomputable def dividedPowers : DividedPowers I
    where
  dpow := dpow I
  dpow_null n x hx := OfInvertibleFactorial.dpow_null hx
  dpow_zero x hx := OfInvertibleFactorial.dpow_zero hx
  dpow_one x hx := OfInvertibleFactorial.dpow_one hx
  dpow_mem n hn x hx := OfInvertibleFactorial.dpow_mem hn hx
  dpow_add n x y hx hy :=
    OfInvertibleFactorial.dpow_add_dif_pos (Factorial.isUnit (n + 1 - 1)) n.lt_succ_self hx hy
  dpow_smul n a x hx := OfInvertibleFactorial.dpow_smul n hx
  dpow_mul m k x hx :=
    OfInvertibleFactorial.dpow_mul_dif_pos (Factorial.isUnit (m + k + 1 - 1)) (m + k).lt_succ_self
      hx
  dpow_comp m k hk x hx :=
...
theorem dividedPowers_unique (hI : DividedPowers I) : hI = dividedPowers I :=
  ...
  rw [divided_powers_dpow_apply, dpow_def n hx, eq_comm, Ring.inverse_mul_eq_iff_eq_mul _ _ _ hn,
