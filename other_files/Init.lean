...
theorem dp_add (n : ℕ) (x y : M) :
  dp R n (x + y) =
    (antidiagonal n).sum fun k => dp R k.1 x * dp R k.2 y := by
  simp only [dp_def]
...
theorem lift'_imp (f : ℕ × M → A)
  ...
  cases' h with a r n a m n a n a b <;>
    simp only [eval₂AlgHom_X', map_one, map_zero, map_smul, _root_.map_mul, map_nsmul, map_sum]
  . apply hf_zero
  . apply hf_smul
  . apply hf_mul
  . apply hf_add
...
lemma LinearMap.dp_add (n : ℕ) (a b : M) :
  dp S n (f (a + b)) =
    (Finset.antidiagonal n).sum fun k => dp S k.1 (f a) * dp S k.2 (f b) := by
