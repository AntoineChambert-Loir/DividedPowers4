...
variable {R A B : Type*} (r : A → A → Prop)
...
variable {β γ : Type*} [AddCommMonoid β] [Module R β] [AddCommMonoid γ] [Module R γ]

instance {F : Type*} [FunLike F β γ] [LinearMapClass F R β γ] :
    CoeTC F (β →ₗ[R] γ) where
  coe h := {
    toFun := h
    map_add' := map_add h
    map_smul' := map_smul h }

theorem LinearMap.coe_coe {F : Type*} [FunLike F β γ] [LinearMapClass F R β γ] (f : F) :
    ((f : β →ₗ[R] γ) : β → γ) = f :=
  rfl
...
example {F : Type*} [FunLike F β γ] [LinearMapClass F R β γ] (h : F) (b : β) :
    (h : β →ₗ[R] γ) b = h b :=
  rfl
...
variable {β γ : Type*} [Semiring β] [Algebra R β] [Semiring γ] [Algebra R γ]

theorem AlgHom.to_linearMap_coe_coe {F : Type*} [FunLike F β γ] [AlgHomClass F R β γ] (h : F) :
    ((h : β →ₗ[R] γ) : β → γ) = h :=
  rfl
...
example {β : Type*} [AddCommMonoid β] : Module ℕ β :=
  AddCommMonoid.natModule

example {β γ : Type*} [AddCommMonoid β] [AddCommMonoid γ] (f : β →+ γ) : β →ₗ[ℕ] γ :=
  { toFun := f
    map_add' := f.map_add
...
example {β γ : Type*} [AddCommMonoid β] [AddCommMonoid γ] (f : β →ₗ[ℕ] γ) : β →+ γ :=
  f.toAddMonoidHom

example {β γ : Type*} [AddCommMonoid β] [AddCommMonoid γ] (f : β →ₗ[ℕ] γ) :
    f = f.toAddMonoidHom.toNatLinearMap :=
  LinearMap.ext fun _ => Eq.refl _

example {β γ : Type*} [AddCommMonoid β] [AddCommMonoid γ] (f : β →+ γ) :
...
example {β : Type*} [AddCommGroup β] : Module ℤ β :=
  AddCommGroup.intModule β

example {β γ : Type*} [AddCommGroup β] [AddCommGroup γ] (f : β →+ γ) : β →ₗ[ℤ] γ :=
  { toFun := f
    map_add' := f.map_add
...
def component' {β : ι → Type*} [∀ i, AddCommMonoid (β i)] (i : ι) :
    (⨁ i, β i) →+ β i :=
  DirectSum.component ℕ ι β i
#align direct_sum.component' DirectSum.component'

theorem component'_eq {β : ι → Type*} [∀ i, AddCommMonoid (β i)] (x : DirectSum ι β)
    (i : ι) : component' i x = x i :=
  rfl
#align direct_sum.component'_eq DirectSum.component'_eq

theorem eq_iff_component'_eq {β : ι → Type*} [∀ i, AddCommMonoid (β i)]
    (x y : ⨁ i, β i) : x = y ↔ ∀ i, component' i x = component' i y :=
  DirectSum.ext_iff ℕ
...
example {β : ι → Type*} [∀ i, AddCommMonoid (β i)] {γ : Type*} [AddCommMonoid γ]
    (h : ∀ i, β i →+ γ) : (⨁ i, β i) →+ γ :=
  DirectSum.toAddMonoid h

-- linear_map from a direct_sum to a module
example {β : ι → Type*} [∀ i, AddCommMonoid (β i)] [∀ i, Module R (β i)] {γ : Type*}
    [AddCommMonoid γ] [Module R γ] (h : ∀ i, β i →ₗ[R] γ) : DirectSum ι β →ₗ[R] γ :=
  DirectSum.toModule R ι γ h

-- linear_map, with classes :
example {β : ι → Type*} [∀ i, AddCommMonoid (β i)] [∀ i, Module R (β i)] {γ : Type*}
    [AddCommMonoid γ] [Module R γ] {F : ∀ _ : ι, Type*}
    [∀ i, FunLike (F i) (β i) γ] [∀ i, LinearMapClass (F i) R (β i) γ] (h : ∀ i, F i) :
    DirectSum ι β →ₗ[R] γ :=
  DirectSum.toModule R ι γ fun i => h i

-- ⟨h i, map_add _, map_smulₛₗ _⟩
example {β : ι → Type*} [∀ i, AddCommMonoid (β i)] [∀ i, Module R (β i)] {γ : Type*}
    [AddCommMonoid γ] [Module R γ] {F : ∀ _ : ι, Type*} [∀ i, FunLike (F i) (β i) γ] [∀ i, LinearMapClass (F i) R (β i) γ]
...
example {β γ : ι → Type*} [∀ i, AddCommMonoid (β i)] [∀ i, AddCommMonoid (γ i)]
...
example {β γ : ι → Type*} [∀ i, AddCommMonoid (β i)]
    -- [Π i, module R (β i)]
    [∀ i, AddCommMonoid (γ i)]
    -- [Π i, module R (γ i)]
    (h : ∀ i, β i →+ γ i) :
...
example {β γ : ι → Type*} [∀ i, AddCommMonoid (β i)] [∀ i, Module R (β i)]
    [∀ i, AddCommMonoid (γ i)] [∀ i, Module R (γ i)] (h : ∀ i, β i →ₗ[R] γ i) :
    DirectSum.lmap' h = DirectSum.lmap h := by rfl

-- Lemmas to help computation
theorem map_of {β γ : ι → Type*}
...
theorem _root_.DFinsupp.mk_eq_sum {β : ι → Type*} [∀ i, AddCommMonoid (β i)]
    [∀ (i : ι) (x : β i), Decidable (x ≠ 0)] (s : Finset ι) (f : ∀ i, β i) :
    (DFinsupp.mk s fun i => f i) = s.sum fun i => of β i (f i) := by
  ext i
  simp only [DFinsupp.mk_apply, DFinsupp.finset_sum_apply]
  split_ifs with hi
  · rw [DFinsupp.finset_sum_apply, Finset.sum_eq_single_of_mem i hi, of_eq_same]
    . intro j _ hij
      rw [of_eq_of_ne]
      exact hij
...
theorem toAddMonoid_mk {β : ι → Type*} [∀ i, AddCommMonoid (β i)]
    {γ : Type*} [AddCommMonoid γ] [∀ (i : ι) (x : β i), Decidable (x ≠ 0)]
    [∀ x : γ, Decidable (x ≠ 0)]
    (ψ : ∀ i, β i →+ γ) (s : Finset ι) (x : ∀ i : s, β i) :
  toAddMonoid ψ (DirectSum.mk β s x) =
    s.sum fun i => ψ i (if h : i ∈ s then x ⟨i, h⟩ else 0) := by
  rw [mk_eq_sum, map_sum]
  apply Finset.sum_congr rfl
  intro i _
  rw [toAddMonoid_of]
#align direct_sum.to_add_monoid_mk DirectSum.toAddMonoid_mk

theorem map'_apply'_old {β γ : ι → Type*} [∀ i, AddCommMonoid (β i)]
    [∀ i, AddCommMonoid (γ i)] [∀ (i : ι) (x : β i), Decidable (x ≠ 0)]
    [∀ (i : ι) (x : γ i), Decidable (x ≠ 0)]
    (h : ∀ i, β i →+ γ i) (x : ⨁ i, β i) :
...
theorem map_apply' {β γ : ι → Type*} [∀ i, AddCommMonoid (β i)]
    [∀ i, AddCommMonoid (γ i)] [∀ (i : ι) (x : β i), Decidable (x ≠ 0)]
    [∀ (i : ι) (x : γ i), Decidable (x ≠ 0)]
    {F : ∀ _, Type*} [∀ i, FunLike (F i) (β i) (γ i)] [∀ i, AddMonoidHomClass (F i) (β i) (γ i)]
    (h : ∀ i, F i) (x : ⨁ i, β i) :
    map h x = mk γ x.support (fun i => (h i) (x i)) := by
...
variable (R : Type*) [CommSemiring R]
variable {ι : Type*} [DecidableEq ι] [AddCommMonoid ι]
variable {A : Type*} [CommSemiring A] [DecidableEq A] [Algebra R A]
variable (𝒜 : ι → Submodule R A)

/- section

variable {σ : Type*} [SetLike σ A] [AddSubmonoidClass σ A] [SMulMemClass σ R A]
...
def RelIsHomogeneous [GradedAlgebra 𝒜] : Prop :=
  ∀ {a b : A} (_ : r a b), ∀ i,
    r ((DirectSum.decomposeAlgEquiv 𝒜).toLinearMap a i)
      ((DirectSum.decomposeAlgEquiv 𝒜).toLinearMap b i)

lemma DirectSum.decomposeAlgEquiv_coe [GradedAlgebra 𝒜] (a : A) :
  DirectSum.decomposeAlgEquiv 𝒜 a = DirectSum.decompose 𝒜 a := by rfl


lemma RelIsHomogeneous_of_isPureHomogeneous [GradedAlgebra 𝒜]
    (hrel : RelIsPureHomogeneous 𝒜 rel) (hrel0 : rel 0 0) :
...
lemma RingConGen.Rel.sum {α : Type*} [Ring α] (r : RingCon α)
    {ι : Type*} [DecidableEq ι] {a b : ι → α} (s : Finset ι)
    (hs : ∀ i ∈ s, r (a i) (b i)) :
...
lemma rel_of_sum_of_rel_add {A : Type*} [AddCommMonoid A]
    (r : A → A → Prop) (hr_zero : r 0 0)
    (hr_add : ∀ {a b c d} (_ : r a c) (_ : r b d), r (a + b) (c + d))
    {ι : Type*} [DecidableEq ι] (f g : ι → A) (s : Finset ι)
    (H : ∀ i ∈ s, r (f i) (g i)) :
...
lemma rel_of_finsupp_sum_of_rel_add {A : Type*} [AddCommMonoid A]
    (r : A → A → Prop) (hr_zero : r 0 0)
    (hr_add : ∀ {a b c d} (_ : r a c) (_ : r b d), r (a + b) (c + d))
    {ι : Type*} [DecidableEq ι] (f g : ι →₀ A)
    (H : ∀ i, r (f i) (g i)) :
...
lemma rel_of_dsum_of_rel_add {A : Type*} [AddCommMonoid A]
    (r : A → A → Prop) (hr_zero : r 0 0)
    (hr_add : ∀ {a b c d} (_ : r a c) (_ : r b d), r (a + b) (c + d))
    {ι : Type*} [DecidableEq ι] {β : ι → Type*}
...
lemma rel_of_dfinsupp_sum_of_rel_add {A : Type*} [AddCommMonoid A]
    (r : A → A → Prop) (hr_zero : r 0 0)
    (hr_add : ∀ {a b c d} (_ : r a c) (_ : r b d), r (a + b) (c + d))
    {ι : Type*} [DecidableEq ι] {β : ι → Type*}
    [∀ i, AddCommMonoid (β i)]
...
variable {R : Type*} [CommRing R]
  ...
  {A : Type*} [CommRing A] [DecidableEq A] [Algebra R A]
...
example (r : R) (a : A) :
  r • (RingQuot.mkAlgHom R rel a) = RingQuot.mkAlgHom R rel (r • a) := by
  simp only [map_smul]
