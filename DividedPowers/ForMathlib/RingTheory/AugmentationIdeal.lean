...
theorem ker_lTensor_of_projectionOnto :
    ker (lTensor Q (Submodule.projectionOnto _ _ hM)) = range (lTensor Q M₂.subtype) := by
  rw [← exact_iff]
  apply lTensor_exact
  simp only [exact_iff, range_subtype, ker_projectionOnto]
  simp only [← range_eq_top, range_projectionOnto]

theorem ker_rTensor_of_projectionOnto :
    ker (rTensor Q (Submodule.projectionOnto _ _ hM)) = range (rTensor Q M₂.subtype) := by
  rw [← exact_iff]
  apply rTensor_exact
  simp only [exact_iff, range_subtype, ker_projectionOnto]
...
theorem isCompl_lTensor (hM : IsCompl M₁ M₂) :
  ...
  have hq :
    M₁.subtype.comp (Submodule.projectionOnto _ _ hM)
      + M₂.subtype.comp (Submodule.projectionOnto _ _ hM.symm) = id := by
    ext x
    simp only [add_apply, coe_comp, coe_subtype, Function.comp_apply, id_coe, id_eq]
    erw [Submodule.projection_add_projection_eq_self]
  apply IsCompl.mk
  · rw [disjoint_def]
    intro x h h'
    ...
    rw [← ker_lTensor_of_projectionOnto hM Q] at h'
    rw [← ker_lTensor_of_projectionOnto hM.symm Q] at h
    rw [mem_ker] at h h'
...
lemma disjoint_left (hM : IsCompl M' M'') :
  ...
  replace h' : x ∈ range (rTensor N M''.subtype) := range_comp_le_range _ _ h'
  rw [← ker_rTensor_of_projectionOnto hM.symm N, mem_ker] at h
  rw [← ker_rTensor_of_projectionOnto hM N, mem_ker] at h'
...
lemma disjoint_right {N' N'' : Submodule A N} (hN : IsCompl N' N'') :
  ...
  replace h' : x ∈ range (lTensor M N''.subtype) := range_comp_le_range _ _ h'
  rw [← ker_lTensor_of_projectionOnto hN.symm M, mem_ker] at h
  rw [← ker_lTensor_of_projectionOnto hN M, mem_ker] at h'
  simp only [h, h', _root_.map_zero, add_zero]
