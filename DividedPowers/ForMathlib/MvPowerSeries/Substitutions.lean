...
def SubstDomain.map {T : Type*} [CommRing T] [Algebra R T] (ε : S →ₐ[R] T) {a : MvPowerSeries τ S}
    (ha : SubstDomain a) : SubstDomain (MvPowerSeries.map τ ε.toRingHom a) where
  const_coeff := by
    obtain ⟨n, hn⟩ := ha.const_coeff
    use n
    simp only [AlgHom.toRingHom_eq_coe, MvPowerSeries.constantCoeff_map, RingHom.coe_coe,
      ← map_pow, hn, map_zero]
