...
theorem topology_eq_ideals_basis_topology :
    MvPowerSeries.WithPiTopology.topologicalSpace (σ := σ) α = (idealIsBasis σ α).topology := by
  let τ := MvPowerSeries.WithPiTopology.topologicalSpace (σ := σ) α
...
-- Alternative proof
lemma has_submodules_basis_topology :
    MvPowerSeries.WithPiTopology.topologicalSpace α = (toSubmodulesBasis σ α).topology := by
  let τ := MvPowerSeries.WithPiTopology.topologicalSpace (σ := σ) α
