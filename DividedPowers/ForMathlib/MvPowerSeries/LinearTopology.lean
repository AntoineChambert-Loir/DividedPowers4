...
/-- The underlying family for the `IdealBasis` in a multivariate power series ring. -/
...
/-- `MvPowerSeries.basis` is an `IdealBasis`. -/
theorem idealBasis : IdealBasis (basis σ α) :=
  IdealBasis.ofComm fun d e => by use d ⊔ e; apply Antitone.map_sup_le (basis_antitone σ α)
#align mv_power_series.ideals_basis MvPowerSeries.idealBasis
...
theorem toRingSubgroupsBasis : RingSubgroupsBasis fun d => (basis σ α d).toAddSubgroup :=
  (idealBasis σ α).toRingSubgroupsBasis
