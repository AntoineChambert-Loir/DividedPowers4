...
theorem AlgHom.factor {R : Type*} [CommRing R] {S : Type*} [CommRing S] [Algebra R S]
...
theorem Ideal.kerLiftAlg_eq_val_comp_Equiv {R : Type*} [CommRing R] {S : Type*} [CommRing S]
  ...
  simp only [AlgHom.coe_comp, Quotient.mkₐ_eq_mk, Function.comp_apply, kerLiftAlg_mk,
    AlgEquiv.toAlgHom_eq_coe, Subalgebra.coe_val, AlgHom.coe_coe]
  rfl
...
def algebraMap' (R : Type*) [CommSemiring R] (S : Type*) [Semiring S] [Algebra R S] :
    R →ₐ[R] S where
  toRingHom := algebraMap R S
  commutes' := fun _ ↦ rfl

lemma TensorProduct.includeRight_lid {R  : Type*} [CommSemiring R]
    {S : Type*} [CommSemiring S] [Algebra R S] {M : Type*} [AddCommMonoid M] [Module R M] (m) :
    (1 : S)  ⊗ₜ[R] (TensorProduct.lid R M) m = (rTensor M (algebraMap' R S).toLinearMap) m := by
  ...
  intro z
  simp only [LinearMap.coe_comp, LinearEquiv.coe_coe, Function.comp_apply, lid_symm_apply, rTensor_tmul,
    AlgHom.toLinearMap_apply, _root_.map_one]
...
noncomputable def toFunLifted :
  ...
  rTensor N (φ R s).toLinearMap (f.toFun' (MvPolynomial (Fin s.card) R) p)
...
theorem exists_lift_of_le_rTensor_range {R : Type*} [CommRing R] (M : Type*) [AddCommGroup M]
    ...
    (A : Subalgebra R T) {φ : S →ₐ[R] T} (hφ : A ≤ φ.range)
    (t : T ⊗[R] M) (ht : t ∈ range ((Subalgebra.val A).toLinearMap.rTensor M)) :
