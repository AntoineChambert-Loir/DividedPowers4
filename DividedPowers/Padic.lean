...
/- Remarks :
1/ can you extraxct the better inequality
    (p - 1) * padicValNat p n.factorial < n
which you prove in the beginning.
It is useful in applications (and it implies, when p ≠ 2, that the p-adic structure
is nilpotent.
...
3/ Do we have ramified extensions of `ℚ_p` ? Then we could say that
the ideal generated by some power `π ^ d` of the uniformizer has a DP structure iff
`d` is large enough (compared to the ramification index).
For example, there is a DP-structure on the maximal ideal iif `e ≤ p - 1`. -/
...
-- ACL : That could be `OnInvertibleFactorial` !
