...
theorem dpowExp_isExponential (hI : DividedPowers I) {a : R} (ha : a ∈ I) :
  ...
  constructor
  · intro p q
    simp only [dpowExp, coeff_mk, hI.dpow_mul _ _ ha]
...
lemma dpowExp_smul (hI : DividedPowers I) {r a : R} (ha : a ∈ I) :
  ...
  ext n
  simp only [dpowExp, coeff_mk, hI.dpow_smul _ ha, coeff_scale, smul_eq_mul]
...
noncomputable def ofExp  [DecidablePred (fun x ↦ x ∈ I)] (e : I →ₗ[R] ExponentialModule R)
  ...
  dpow_null _ _ ha   := by simp only [dif_neg ha]
  ...
  dpow_mem _ _ hn ha := by simp only [dif_pos ha]; exact coeff_mem _ hn
  ...
  dpow_mul _ _ _ ha := by simp only [dif_pos ha, add_mul_coe']
