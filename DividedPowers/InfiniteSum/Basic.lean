...
@[to_additive tsum "`∑' i, f i` is the sum of `f` it exists, or 0 otherwise"]
...
theorem hasProd_one : HasProd (fun b => 1 : β → α) 1 := by simp [HasProd, tendsto_const_nhds]
#align has_prod_one hasProd_one
#align has_sum_zero hasSum_zero

@[to_additive]
theorem hasProd_empty [IsEmpty β] : HasProd f 1 := by convert hasProd_one
#align has_prod_empty hasProd_empty
#align has_sum_empty hasSum_empty

--  summable_zero]
@[to_additive]
theorem multipliable_one : Multipliable (fun b => 1 : β → α) :=
...
@[to_additive HasSum.hasSum_of_sum_eq]
...
theorem Function.Injective.hasProd_iff {g : γ → β} (hg : Injective g)
  ...
  simp only [HasProd, tendsto, hg.map_at_top_finset_prod_eq hf]
...
theorem hasProd_subtype_iff_of_mulSupport_subset {s : Set β} (hf : mulSupport f ⊆ s) :
    HasProd (f ∘ coe : s → α) a ↔ HasProd f a :=
  Subtype.coe_injective.hasProd_iff <| by simpa using mul_support_subset_iff'.1 hf
...
theorem hasProd_subtype_iff_mulIndicator {s : Set β} :
    HasProd (f ∘ coe : s → α) a ↔ HasProd (s.mulIndicator f) a := by
...
theorem multipliable_subtype_iff_mulIndicator {s : Set β} :
    Multipliable (f ∘ coe : s → α) ↔ Multipliable (s.mulIndicator f) :=
...
@[to_additive, simp]
theorem hasProd_subtype_mulSupport : HasProd (f ∘ coe : mulSupport f → α) a ↔ HasProd f a :=
...
theorem Finset.hasProd (s : Finset β) (f : β → α) :
    HasProd (f ∘ coe : (↑s : Set β) → α) (∏ b in s, f b) := by rw [← prod_attach];
...
theorem Finset.multipliable (s : Finset β) (f : β → α) :
...
theorem Set.Finite.multipliable {s : Set β} (hs : s.Finite) (f : β → α) :
    Multipliable (f ∘ coe : s → α) := by
  convert hs.to_finset.multipliable f <;> simp only [hs.coe_to_finset]
...
@[to_additive]
theorem hasProd_pi_mulSingle [DecidableEq β] (b : β) (a : α) : HasProd (Pi.mulSingle b a) a :=
  show HasProd (fun x => Pi.mulSingle b a x) a by
    simpa only [Pi.mulSingle_apply] using hasProd_ite_eq b a
...
theorem Equiv.multipliable_iff (e : γ ≃ β) : Multipliable (f ∘ e) ↔ Multipliable f :=
  exists_congr fun a => e.hasProd_iff
...
theorem Multipliable.prod_symm {f : β × γ → α} (hf : Multipliable f) :
    Multipliable fun p : γ × β => f p.symm :=
...
theorem Equiv.hasProd_iff_of_mulSupport {g : γ → α} (e : mulSupport f ≃ mulSupport g)
  ...
  rw [← hasProd_subtype_mulSupport, ← this, e.has_prod_iff, hasProd_subtype_mulSupport]
...
@[protected, to_additive]
theorem HasProd.map [CommMonoid γ] [TopologicalSpace γ] (hf : HasProd f a) {G}
...
@[protected, to_additive]
theorem Multipliable.map [CommMonoid γ] [TopologicalSpace γ] (hf : Multipliable f) {G}
...
@[protected, to_additive]
theorem Multipliable.map_iff_of_leftInverse [CommMonoid γ] [TopologicalSpace γ] {G G'}
...
@[protected, to_additive]
theorem Multipliable.map_iff_of_equiv [CommMonoid γ] [TopologicalSpace γ] {G} [MulEquivClass G α γ]
    (g : G) (hg : Continuous g) (hg' : Continuous (MulEquivClass.inv g : γ → α)) :
    Multipliable (g ∘ f) ↔ Multipliable f :=
  Multipliable.map_iff_of_leftInverse g (g : α ≃* γ).symm hg hg' (MulEquivClass.left_inv g)
...
theorem Multipliable.hasProd_iff_tendsto_nat [T2Space α] {f : ℕ → α} {a : α} (hf : Multipliable f) :
    HasProd f a ↔ Tendsto (fun n : ℕ => ∏ i in range n, f i) atTop (𝓝 a) :=
  by
...
#align function.surjective.multipliable_iff_of_has_sum_iff Function.Surjective.multipliable_iff_of_has_sum_iff

variable [ContinuousMul α]

@[to_additive HasSum.add]
theorem HasProd.mul (hf : HasProd f a) (hg : HasProd g b) : HasProd (fun b => f b * g b) (a * b) :=
  by simp only [HasProd, prod_mul_distrib] <;> exact hf.mul hg
...
theorem hasProd_mul {f : γ → β → α} {a : γ → α} {s : Finset γ} :
  ...
  Finset.induction_on s (by simp only [hasProd_one, prod_empty, forall_true_iff])
    (by
      simp (config := { contextual := true }) only [HasProd.mul, prod_insert, mem_insert,
        forall_eq_or_imp, forall₂_true_iff, not_false_iff, forall_true_iff])
#align has_prod_mul hasProd_mul
#align has_sum_add has_sum_add
...
#align multipliable_sum multipliable_sum

@[to_additive]
theorem HasProd.mul_disjoint {s t : Set β} (hs : Disjoint s t) (ha : HasProd (f ∘ coe : s → α) a)
    (hb : HasProd (f ∘ coe : t → α) b) : HasProd (f ∘ coe : s ∪ t → α) (a * b) :=
  by
...
theorem hasProd_prod_disjoint {ι} (s : Finset ι) {t : ι → Set β} {a : ι → α}
    (hs : (s : Set ι).Pairwise (Disjoint on t)) (hf : ∀ i ∈ s, HasProd (f ∘ coe : t i → α) (a i)) :
    HasProd (f ∘ coe : (⋃ i ∈ s, t i) → α) (∏ i in s, a i) :=
  by
...
@[to_additive]
theorem HasProd.mul_isCompl {s t : Set β} (hs : IsCompl s t) (ha : HasProd (f ∘ coe : s → α) a)
    (hb : HasProd (f ∘ coe : t → α) b) : HasProd f (a * b) := by
...
@[to_additive]
theorem HasProd.mul_compl {s : Set β} (ha : HasProd (f ∘ coe : s → α) a)
    (hb : HasProd (f ∘ coe : sᶜ → α) b) : HasProd f (a * b) :=
...
theorem Multipliable.mul_compl {s : Set β} (hs : Multipliable (f ∘ coe : s → α))
    (hsc : Multipliable (f ∘ coe : sᶜ → α)) : Multipliable f :=
...
theorem HasProd.compl_mul {s : Set β} (ha : HasProd (f ∘ coe : sᶜ → α) a)
    (hb : HasProd (f ∘ coe : s → α) b) : HasProd f (a * b) :=
...
theorem HasProd.even_mul_odd {f : ℕ → α} (he : HasProd (fun k => f (2 * k)) a)
    (ho : HasProd (fun k => f (2 * k + 1)) b) : HasProd f (a * b) :=
  by
...
theorem Multipliable.compl_mul {s : Set β} (hs : Multipliable (f ∘ coe : sᶜ → α))
    (hsc : Multipliable (f ∘ coe : s → α)) : Multipliable f :=
...
#align tprod_eq_sum tprod_eq_sum
...
theorem prod_eq_tprod_mulIndicator (f : β → α) (s : Finset β) :
  ...
  have : ∀ (x) (_ : x ∉ s), Set.mulIndicator (↑s) f x = 1 := fun x hx =>
...
#align sum_eq_tprod_indicator sum_eq_tprod_indicator
...
/- ./././Mathport/Syntax/Translate/Basic.lean:638:2: warning: expanding binder collection (b' «expr ≠ » b) -/
@[to_additive]
theorem tprod_eq_mul_single {f : β → α} (b : β) (hf : ∀ (b') (_ : b' ≠ b), f b' = 1) :
    ∏' b, f b = f b :=
  (hasProd_mul_single b hf).tprod_eq
#align tprod_eq_mul_single tprod_eq_mul_single
#align tprod_eq_single tprod_eq_single

/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (b' c') -/
/- ./././Mathport/Syntax/Translate/Basic.lean:638:2: warning: expanding binder collection (b' «expr ≠ » b) -/
/- ./././Mathport/Syntax/Translate/Expr.lean:107:6: warning: expanding binder group (b' c') -/
@[to_additive]
theorem tprod_tprod_eq_mul_single (f : β → γ → α) (b : β) (c : γ)
    (hfb : ∀ (b') (_ : b' ≠ b), f b' c = 1) (hfc : ∀ (b' : β) (c' : γ), c' ≠ c → f b' c' = 1) :
    ∏' (b') (c'), f b' c' = f b c :=
  calc
    ∏' (b') (c'), f b' c' = ∏' b', f b' c := tprod_congr fun b' => tprod_eq_mul_single _ (hfc b')
    _ = f b c := tprod_eq_mul_single _ hfb
#align tprod_tprod_eq_mul_single tprod_tprod_eq_mul_single
#align tprod_tprod_eq_single tprod_tprod_eq_single

@[simp, to_additive tsum_ite_eq]
...
#align tprod_pi_single tprod_pi_single
...
theorem Function.Surjective.tprod_eq_tprod_of_hasProd_iff_hasProd {α' : Type _} [CommMonoid α']
  ...
  by_cases (fun this : Multipliable g => (h.mpr this.HasProd).tprod_eq) fun hg : ¬Multipliable g =>
    by
    have hf : ¬Multipliable f := mt (hes.multipliable_iff_of_hasProd_iff @h).1 hg
    simp [tprod, hf, hg, h1]
#align function.surjective.tprod_eq_tprod_of_has_prod_iff_has_prod Function.Surjective.tprod_eq_tprod_of_hasProd_iff_hasProd
#align function.surjective.tprod_eq_tprod_of_has_sum_iff_has_sum Function.Surjective.tprod_eq_tprod_of_has_sum_iff_has_sum
...
#align tprod_eq_tprod_of_has_sum_iff_has_sum tprod_eq_tprod_of_has_sum_iff_has_sum

@[to_additive Equiv.tsum_eq]
theorem Equiv.tprod_eq (j : γ ≃ β) (f : β → α) : ∏' c, f (j c) = ∏' b, f b :=
  tprod_eq_tprod_of_hasProd_iff_hasProd fun a => j.hasProd_iff
...
theorem Equiv.tprod_eq_tprod_of_mulSupport {f : β → α} {g : γ → α} (e : mulSupport f ≃ mulSupport g)
  ...
  tprod_eq_tprod_of_hasProd_iff_hasProd fun _ => e.hasProd_iff_of_mulSupport he
#align equiv.tprod_eq_tprod_of_mul_support Equiv.tprod_eq_tprod_of_mulSupport
#align equiv.tsum_eq_tsup_of_support Equiv.tsum_eq_tsup_of_support

@[to_additive]
theorem tprod_eq_tprod_of_ne_one_bij {g : γ → α} (i : mulSupport g → β)
    (hi : ∀ ⦃x y⦄, i x = i y → (x : γ) = y) (hf : mulSupport f ⊆ Set.range i)
    (hfg : ∀ x, f (i x) = g x) : ∏' x, f x = ∏' y, g y :=
  tprod_eq_tprod_of_hasProd_iff_hasProd fun _ => hasProd_iff_hasProd_of_ne_one_bij i hi hf hfg
#align tprod_eq_tprod_of_ne_one_bij tprod_eq_tprod_of_ne_one_bij
#align tprod_eq_tprod_of_ne_zero_bij tprod_eq_tprod_of_ne_zero_bij
...
theorem tprod_subtype (s : Set β) (f : β → α) : ∏' x : s, f x = ∏' x, s.mulIndicator f x :=
  tprod_eq_tprod_of_hasProd_iff_hasProd fun _ => hasProd_subtype_iff_mulIndicator
...
theorem tprod_subtype_eq_of_mulSupport_subset {f : β → α} {s : Set β} (hs : mulSupport f ⊆ s) :
    ∏' x : s, f x = ∏' x, f x :=
  tprod_eq_tprod_of_hasProd_iff_hasProd fun x => hasProd_subtype_iff_of_mulSupport_subset hs
...
theorem tprod_singleton (b : β) (f : β → α) : ∏' x : ({b} : Set β), f x = f b :=
  by
  rw [tprod_subtype, tprod_eq_mul_single b]
  · simp
  · intro b' hb'
    rw [Set.mulIndicator_of_not_mem]
...
theorem tprod_range {g : γ → β} (f : β → α) (hg : Injective g) :
  ...
  rw [← Set.image_univ, tprod_image f (hg.inj_on _), tprod_univ (f ∘ g)]
...
theorem tprod_eq_mul_tprod_ite' {f : β → α} (b : β) (hf : Multipliable (f.update b 1)) :
    ∏' x, f x = f b * ∏' x, ite (x = b) 1 (f x) :=
  by
  have : ∏' x, f x = ∏' x, ite (x = b) (f x) 1 * f.update b 1 x := by apply tprod_congr; intro c;
    split_ifs <;> simp [Function.update_apply, h]
  rw [this]
  rw [tprod_mul _ hf]
  congr
  rw [tprod_eq_mul_single b]; simp only [eq_self_iff_true, if_true]
  · intro c hc; rw [if_neg hc]
  infer_instance
  simp only [Function.update_apply]
  exact ⟨ite (b = b) (f b) 1, hasProd_mul_single b fun b hb => if_neg hb⟩
#align tprod_eq_mul_tprod_ite' tprod_eq_mul_tprod_ite'
#align tprod_eq_add_tprod_ite' tprod_eq_add_tprod_ite'
...
@[to_additive tsum_iSup_decode₂]
theorem tprod_iSup_decode₂ [CompleteLattice β] (m : β → α) (m1 : m ⊥ = 1) (s : γ → β) :
    ∏' i : ℕ, m (⨆ b ∈ decode₂ γ i, s b) = ∏' b : γ, m (s b) :=
  by
  have H : ∀ n, m (⨆ b ∈ decode₂ γ n, s b) ≠ 1 → (decode₂ γ n).isSome :=
    by
    intro n h
    cases' decode₂ γ n with b
    · refine' (h <| by simp [m1]).elim
    · exact rfl
  symm; refine' tprod_eq_tprod_of_ne_one_bij (fun a => Option.get (H a.1 a.2)) _ _ _
  · rintro ⟨m, hm⟩ ⟨n, hn⟩ e
...
theorem rel_iSup_tprod [CompleteLattice β] (m : β → α) (m1 : m ⊥ = 1) (R : α → α → Prop)
  ...
  rw [← supr_decode₂, ← tprod_iSup_decode₂ _ m1 s]; exact m_supr _
...
theorem rel_iSup_prod [CompleteLattice β] (m : β → α) (m1 : m ⊥ = 1) (R : α → α → Prop)
    ...
    R (m (⨆ d ∈ t, s d)) (∏ d in t, m (s d)) := by rw [iSup_subtype', ← Finset.tprod_subtype];
...
theorem tprod_mul_tprod_compl {s : Set β} (hs : Multipliable (f ∘ coe : s → α))
    (hsc : Multipliable (f ∘ coe : sᶜ → α)) : (∏' x : s, f x) * ∏' x : sᶜ, f x = ∏' x, f x :=
...
theorem tprod_union_disjoint {s t : Set β} (hd : Disjoint s t) (hs : Multipliable (f ∘ coe : s → α))
    (ht : Multipliable (f ∘ coe : t → α)) : ∏' x : s ∪ t, f x = (∏' x : s, f x) * ∏' x : t, f x :=
...
theorem tprod_finset_bUnion_disjoint {ι} {s : Finset ι} {t : ι → Set β}
    (hd : (s : Set ι).Pairwise (Disjoint on t)) (hf : ∀ i ∈ s, Multipliable (f ∘ coe : t i → α)) :
...
@[to_additive HasSum.hasSum_compl_iff]
theorem HasProd.hasProd_compl_iff {s : Set β} (hf : HasProd (f ∘ coe : s → α) a₁) :
    HasProd (f ∘ coe : sᶜ → α) a₂ ↔ HasProd f (a₁ * a₂) :=
  by
...
@[to_additive HasSum.hasSum_iff_compl]
theorem HasProd.hasProd_iff_compl {s : Set β} (hf : HasProd (f ∘ coe : s → α) a₁) :
    HasProd f a₂ ↔ HasProd (f ∘ coe : sᶜ → α) (a₂ / a₁) :=
...
theorem Multipliable.multipliable_compl_iff {s : Set β} (hf : Multipliable (f ∘ coe : s → α)) :
    Multipliable (f ∘ coe : sᶜ → α) ↔ Multipliable f :=
  ⟨fun ⟨a, ha⟩ => (hf.HasProd.hasProd_compl_iff.1 ha).Multipliable, fun ⟨a, ha⟩ =>
...
@[protected, to_additive Finset.hasSum_compl_iff]
theorem Finset.hasProd_compl_iff (s : Finset β) :
...
@[protected, to_additive Finset.hasSum_iff_compl]
theorem Finset.hasProd_iff_compl (s : Finset β) :
...
theorem Finset.multipliable_compl_iff (s : Finset β) :
...
theorem tprod_eq_mul_tprod_ite [DecidableEq β] (hf : Multipliable f) (b : β) :
  ...
  by
  rw [(hasProd_ite_div_hasProd hf.has_prod b).tprod_eq]
...
theorem hasProd_nat_add_iff {f : ℕ → α} (k : ℕ) {a : α} :
    HasProd (fun n => f (n + k)) a ↔ HasProd f (a * ∏ i in range k, f i) :=
  by
...
theorem multipliable_nat_add_iff {f : ℕ → α} (k : ℕ) :
  ...
  Iff.symm <|
    (Equiv.mulRight (∏ i in range k, f i)).Surjective.multipliable_iff_of_hasProd_iff fun a =>
      (hasProd_nat_add_iff k).symm
#align multipliable_nat_add_iff multipliable_nat_add_iff
...
theorem prod_mul_tprod_nat_add [T2Space α] {f : ℕ → α} (k : ℕ) (h : Multipliable f) :
  ...
  simpa only [mul_comm] using
    ((hasProd_nat_add_iff k).1 ((multipliable_nat_add_iff k).2 h).HasProd).unique h.has_prod
...
theorem HasProd.int_rec {b : α} {f g : ℕ → α} (hf : HasProd f a) (hg : HasProd g b) :
  ...
  exact HasProd.mul_isCompl this (h₁.has_prod_range_iff.mpr hf) (h₂.has_prod_range_iff.mpr hg)
...
@[to_additive HasSum.pos_mul_zero_add_neg]
theorem HasProd.pos_add_zero_add_neg {b : α} {f : ℤ → α} (hpos : HasProd (fun n : ℕ => f (n + 1)) a)
    (hneg : HasProd (fun n : ℕ => f (-n.succ)) b) : HasProd f (a * f 0 * b) :=
  haveI : ∀ g : ℕ → α, HasProd (fun k => g (k + 1)) a → HasProd g (a * g 0) := by intro g hg;
    simpa using (hasProd_nat_add_iff _).mp hg
  (this (fun n => f n) hpos).nonneg_add_neg hneg
#align has_prod.pos_add_zero_add_neg HasProd.pos_add_zero_add_neg
#align has_sum.pos_mul_zero_add_neg HasSum.pos_mul_zero_add_neg
...
theorem cauchySeq_finset_iff_mul_vanishing :
...
theorem tendsto_tprod_compl_atTop_one (f : β → α) :
    Tendsto (fun s : Finset β => ∏' b : { x // x ∉ s }, f b) atTop (𝓝 1) :=
  by
  ...
  · intro e he
    ...
    obtain ⟨s, hs⟩ : ∃ s : Finset β, ∀ t : Finset β, Disjoint t s → ∏ b : β in t, f b ∈ o :=
      cauchySeq_finset_iff_mul_vanishing.1 (tendsto.cauchy_seq H.has_prod) o ho
    refine' ⟨s, fun a sa => oe _⟩
    have A : Multipliable fun b : { x // x ∉ a } => f b := a.multipliable_compl_iff.2 H
    apply IsClosed.mem_of_tendsto o_closed A.has_prod (eventually_of_forall fun b => _)
...
theorem Multipliable.mulIndicator (hf : Multipliable f) (s : Set β) :
...
theorem Multipliable.comp_injective {i : γ → β} (hf : Multipliable f) (hi : Injective i) :
  ...
  simpa only [Set.mulIndicator_range_comp] using
    (hi.multipliable_iff _).2 (hf.mul_indicator (Set.range i))
  exact fun x hx => Set.mulIndicator_of_not_mem hx _
#align multipliable.comp_injective Multipliable.comp_injective
#align summable.comp_injective Summable.comp_injective

@[to_additive]
theorem Multipliable.subtype (hf : Multipliable f) (s : Set β) : Multipliable (f ∘ coe : s → α) :=
...
theorem Multipliable.mul_vanishing (hf : Multipliable f) ⦃e : Set G⦄ (he : e ∈ 𝓝 (1 : G)) :
  ...
  rcases hf with ⟨y, hy⟩
  exact cauchySeq_finset_iff_mul_vanishing.1 hy.cauchy_seq e he
...
@[to_additive Summable.tendsto_atTop_zero]
