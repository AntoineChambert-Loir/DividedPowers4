...
lemma algebraMap_dpow_mem (x : I) {n : ℕ} (hn : n ≠ 0) :
  ...
  apply hIJ
  simp only [Algebra.algebraMap_self, RingHomCompTriple.comp_apply]
  exact Ideal.mem_map_of_mem _ (dpow_mem _ hn x.2)

-- (DividedPowerAlgebra.embed B J x) is the map φ in B-O
-- (i)
...
inductive rel1 : Rel (DividedPowerAlgebra B J) (DividedPowerAlgebra B J)
  | Rel {x : J} : rel1 (embed B J x) (algebraMap _ _ (x : B))

variable {J} in
lemma rel1_apply (x : J) :
  rel1 J ((embed B ↥J) x) ((algebraMap B (DividedPowerAlgebra B ↥J)) ↑x) := by constructor
...
lemma rel1_mem_J1 (x : J) :
    ((embed B ↥J) x - ((algebraMap B (DividedPowerAlgebra B ↥J)) ↑x)) ∈ J1 J := by
  apply subset_span
  use (embed B ↥J) x, (algebraMap B (DividedPowerAlgebra B ↥J)) ↑x
  constructor
  · exact rel1_apply x
...
inductive rel2' : Rel (DividedPowerAlgebra B J) (DividedPowerAlgebra B J)
  | Rel {x : I} {n : ℕ} (hn : n ≠ 0) :
    rel2' (dp B n (⟨algebraMap A B x, hIJ (Ideal.mem_map_of_mem _ x.2)⟩ : J))
      (embed B J ⟨(algebraMap A B (dpow hI n x)), algebraMap_dpow_mem hI J hIJ x hn⟩)
...
lemma rel1_mem_J12 (x : J) :
    ((embed B ↥J) x - ((algebraMap B (DividedPowerAlgebra B ↥J)) ↑x)) ∈ J12 hI J hIJ :=
...
lemma rel2_mem_J12 (x : I) {n : ℕ} (hn : n ≠ 0) :
  ...
  | Rel {x : I} {n : ℕ} (hn : n ≠ 0) :
    rel2' (dp B n (⟨algebraMap A B x, hIJ (Ideal.mem_map_of_mem _ x.2)⟩ : J))
      (embed B J ⟨(algebraMap _ _ (algebraMap A B (dpow hI n x))), algebraMap_dpow_mem hI J hIJ x hn⟩)-/
...
theorem sub_ideal_dpIdealOfIncluded [DecidableEq B] :
  ...
  · rw [← hyx]
    ...
    · intro x hx
      ...
      obtain ⟨y, hyJ, hyx⟩ := hx
      have hsub : (((algebraMap B (DividedPowerAlgebra B ↥J)) y) - (embed B ↥J) ⟨y, hyJ⟩) ∈
          J12 hI J hIJ := by
        rw [← neg_sub, Ideal.neg_mem_iff]
        exact rel1_mem_J12 hI J hIJ ⟨y, hyJ⟩
      rw [Submodule.mem_sup]
      use ((embed B ↥J) ⟨y, hyJ⟩), dp_mem_augIdeal B ↥J zero_lt_one ⟨y, hyJ⟩,
        (((algebraMap B (DividedPowerAlgebra B ↥J)) y) - (embed B ↥J) ⟨y, hyJ⟩), hsub
      rw [← hyx]
      ring
...
noncomputable def dpIdeal : (dividedPowers' hI J).SubDPIdeal :=
  sorry --SubDPIdeal.dpowGenerated (dividedPowers' hI J) (J_le_J₁_bar hI J)
...
private noncomputable abbrev ψ [DecidableEq B] [∀ x, Decidable (x ∈ (dpIdeal hI J).carrier)] :
  ...
  toRingHom  := sorry --(φ hI J hK hJK hI' hI'_ext hI'_int).toRingHom
  ideal_comp := by
  ...
  dpow_comp  := fun {n} a ha => by
    ...
    rw [← IdealAdd.dpow_eq_of_mem_right hI'_int]
    · sorry
...
private noncomputable def foo [DecidableEq B] :
  ...
  sorry --DPMorphism.mk' (Quotient.isDPMorphism (DividedPowerAlgebra.dividedPowers B ↥(J' I J)) _)
