...
@[implicit_reducible]
def DividedPowerAlgebra := RingQuot (DividedPowerAlgebra.Rel R M)

namespace DividedPowerAlgebra

instance : CommSemiring (DividedPowerAlgebra R M) := RingQuot.instCommSemiring (Rel R M)

instance {R M : Type*} [CommRing R] [AddCommMonoid M] [Module R M] :
    CommRing (DividedPowerAlgebra R M) := RingQuot.instCommRing (Rel R M)

instance : Algebra R (DividedPowerAlgebra R M) := RingQuot.instAlgebra (Rel R M)

--variable {S : Type*} [CommSemiring S] [Module S M] [Algebra R S] [IsScalarTower R S M]

--instance : Algebra R (DividedPowerAlgebra S M) := RingQuot.instAlgebra (Rel S M)

--instance : IsScalarTower R S (DividedPowerAlgebra S M) := RingQuot.instIsScalarTower (Rel S M)
