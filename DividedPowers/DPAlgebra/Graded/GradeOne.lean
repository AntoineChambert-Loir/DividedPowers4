...
* `DividedPowerAlgebra.toTrivSqZeroExt` : the canonical map from `DividedPowerAlgebra R M` into
  `TrivSqZeroExt R M` that sends `DividedPowerAlgebra.embed` to `TrivSqZeroExt.inr`.
...
theorem embed_mem_grade_one (m : M) : embed R M m ∈ grade R M 1 :=
...
@[simp] theorem toTrivSqZeroExt_embed (x : M) :
    toTrivSqZeroExt R M (embed R M x) = inr x := lift_embed_apply _ _ x
...
lemma embed_toTrivSqZeroExt_of_mem_grade_one {a} (ha : a ∈ grade R M 1) :
    (embed R M) ((sndHom R M) ((toTrivSqZeroExt R M) a)) = a := by
  suffices ⟨(embed R M) ((sndHom R M) ((toTrivSqZeroExt R M) a)), embed_mem_grade_one R _⟩ =
      (⟨a, ha⟩ : grade R M 1) by
...
theorem mem_grade_one_iff (a : DividedPowerAlgebra R M) :
  ...
  . intro ha
    ...
    rw [embed_toTrivSqZeroExt_of_mem_grade_one R M ha]
  . rintro ⟨m, rfl⟩
    exact embed_mem_grade_one R m
