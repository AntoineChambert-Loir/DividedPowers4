...
@[simp]
theorem coe_zero : (↑(0 : 𝒜 0) : A) = 0 := rfl
...
@[simp]
theorem coe_mul (x y : 𝒜 0) : (↑(x * y) : A) = x * y := rfl

@[simp]
theorem val_mul (x y : 𝒜 0) : (x * y).val = x.val * y.val := rfl

@[simp]
theorem coe_one : (↑(1 : 𝒜 0) : A) = 1 := rfl
...
instance algebra : Algebra R (𝒜 0) :=
  Algebra.ofModule'
    (fun r x => by
      ext
      simp only [coe_mul, coe_smul, SetLike.val_smul, coe_one, Algebra.smul_mul_assoc, one_mul])
    (fun r x => by
      ext
      simp only [coe_mul, coe_smul, SetLike.val_smul, coe_one, Algebra.mul_smul_comm, mul_one])
...
def proj'_zeroRingHom : A →+* 𝒜 0 where
  ...
  map_mul' x y := by
    ...
    simp only [proj', LinearMap.coe_mk, AddHom.coe_mk, SetLike.coe_eq_coe,
      GradeZero.coe_mul, ← GradedRing.projZeroRingHom_apply 𝒜, ← _root_.map_mul]
