...
variable {ι : Type _} [CanonicallyOrderedAddCommMonoid ι]

variable (𝒜 : ι → Submodule R A)

theorem gradeZero_coe_smul (r : R) (x : 𝒜 0) : (↑(r • x) : A) = r • x := rfl
...
@[simp]
theorem gradeZero_coe_mul (x y : 𝒜 0) : (↑(x * y) : A) = x * y := rfl

@[simp]
theorem gradeZero_val_mul (x y : 𝒜 0) : (x * y).val = x.val * y.val := rfl

omit [DecidableEq ι] [GradedAlgebra 𝒜] in
@[simp]
theorem gradeZero_coe_zero : (↑(0 : 𝒜 0) : A) = 0 := rfl

@[simp]
theorem gradeZero_coe_one : (↑(1 : 𝒜 0) : A) = 1 := rfl
