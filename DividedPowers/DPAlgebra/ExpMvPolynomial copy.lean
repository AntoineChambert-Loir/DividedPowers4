...
lemma dpowMonomial'_linearIndependent [Algebra ℚ K] [Nontrivial K] [IsDomain R]
    [IsTorsionFree R K] :
    LinearIndependent R (fun (n : ℕ) ↦ (1/(n.factorial : ℚ)) • (X : K[X])^n) := by
  rw [linearIndependent_iff]
  intro l hl
  simp only [one_div, Finsupp.linearCombination_apply] at hl
  simp only [Finsupp.sum, ext_iff, finset_sum_coeff, coeff_smul, coeff_X_pow, smul_ite, smul_zero,
    sum_ite_eq, Finsupp.mem_support_iff, ne_eq, coeff_zero, ite_eq_right_iff, smul_eq_zero,
    inv_eq_zero, Nat.cast_eq_zero, one_ne_zero, or_false] at hl
  ext n
  aesop (add norm Nat.factorial_ne_zero)

variable {M : Type*} [AddCommGroup M] [Module R M] {ι : Type*} [Unique ι] (b : Basis ι R M)
  [Algebra ℚ K] -- Should this already exist as a definition or as an instance?

variable [Nontrivial K] [IsDomain R] [IsTorsionFree R K]

def expBasis [Nontrivial K] [IsDomain R] [IsTorsionFree R K] :
    Basis ℕ R (expPolynomial R K) :=
  Module.Basis.span (dpowMonomial'_linearIndependent R K)
...
def morphismInv : expPolynomial R K →ₗ[R] DividedPowerAlgebra R M :=
  (expBasis R K).constr R (fun k ↦ dp R k (b default))

lemma morphism_comp_morphismInv_eq_val :
    ((morphism K b).toLinearMap.comp (morphismInv K b)) =
      (expPolynomial R K).val := by
  apply (expBasis R K).ext
  intro n
  simp only [LinearMap.coe_comp, AlgHom.coe_toLinearMap, Function.comp_apply,
    ...
    expBasis]
  erw [Basis.span_apply (dpowMonomial'_linearIndependent R K) n] --Why erw?
  simp only [morphismInv]
...
def expMvBasis [Nontrivial K] [IsDomain R] [IsTorsionFree R K] :
