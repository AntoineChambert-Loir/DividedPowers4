...
theorem mul_dp_mem_span (n : ι →₀ ℕ) (k : ℕ) (i : ι) :
    (n.prod fun i k ↦ dp R k (v i)) * dp R k (v i) ∈
...
theorem mul_dp_mem_span_of_mem_span (m : DividedPowerAlgebra R M)
    (hm : m ∈ Submodule.span R (Set.range fun (n : ι →₀ ℕ) ↦ n.prod fun i k ↦ dp R k (v i)))
    (k : ℕ) (i : ι) :
    m * dp R k (v i) ∈ Submodule.span R (Set.range fun (n : ι →₀ ℕ) ↦ n.prod fun i k ↦ dp R k (v i)) := by
...
theorem span_prod_dp_eq_top :
  ...
  | h_dp x k m hx =>
    ...
    | mem x hx =>
      obtain ⟨n, rfl⟩ := hx
      simp only
      induction hm using Submodule.span_induction generalizing k n with
      | mem m hm =>
        obtain ⟨i, rfl⟩ := hm
        exact mul_dp_mem_span n k i
      | zero =>
        rw [dp_null]
        split_ifs with hk
        · apply Submodule.mem_span_of_mem
      ...
      | add m m' _ _ hm hm' =>
        ...
        rw [← mul_assoc]
        specialize hm c.1 n
        induction m using Submodule.span_induction hm
        -- Won't work
        sorry
      | smul => sorry
    | zero => sorry
    ...
    | insert i s hi hind =>
      ...
      specialize hind (subset_trans (by simp) hs)
      sorry -/
...
lemma mk_X {R : Type u} [CommRing R] {M : Type v} [AddCommGroup M] [Module R M]
    (mn : ℕ × M) : mk (X mn) = dp R mn.1 mn.2 := by
  simp [dp]; rfl

--TODO: rename dp_ lemmas to _dp.
open Nat in
lemma prod_dp {R : Type u} [CommRing R] {M : Type v} [AddCommGroup M] [Module R M]
    {ι : Type*} (s : Finset ι) {n : ι → ℕ} (m : M) :
