...
instance : Semiring (DividedPowerAlgebra R M) := RingQuot.instSemiring _

-- `IsScalarTower` is not needed, but the instance isn't really canonical without it.
--@[nolint unusedArguments]
instance instAlgebra  {R A M} [CommSemiring R] [AddCommMonoid M] [CommRing A]
...
instance {R S A M} [CommRing R] [CommRing S] [AddCommMonoid M] [CommRing A]
...
instance {S : Type _} [CommRing S] [Module S M] : Ring (DividedPowerAlgebra S M) :=
...
example : Zero (DividedPowerAlgebra R M) := by exact Zero.ofOfNat0

example : One (DividedPowerAlgebra R M) := by exact Monoid.toOne
