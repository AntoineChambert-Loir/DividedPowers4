...
variable [Algebra R (DividedPowerAlgebra S N)] [IsScalarTower R S (DividedPowerAlgebra S N)]
...
private lemma id_eq_lift_toN (x : DividedPowerAlgebra R M) :
  ...
  intro d
  rw [toN, Finsupp.linearCombination_apply, map_finsuppSum]
  simp only [map_smul, lift_f]
  simp only [map_finsuppSum, Finsupp.sum_apply]
  classical
  have (k : ι →₀ ℕ) (p : MvPolynomial R ℤ) :
    ((basis R M b).repr (p • (basis R M b) k)) d =
    if k = d then (algebraMap (MvPolynomial R ℤ) R) p else 0 := by
    rw [algebra_compatible_smul R p, map_smul]
    simp [Algebra.algebraMap_eq_smul_one]
    simp [Finsupp.single_apply]
  simp_rw [this]
  simp only [Finsupp.sum_ite_eq', Finsupp.mem_support_iff, Finsupp.mapRange_apply, ne_eq, ite_not, X']
...
private lemma id_eq_lift_toN' (x : DividedPowerAlgebra R M) :
  ...
  simp only [
    LinearMap.id_coe, id_eq, map_finsuppSum, map_smul, lift_f, Finsupp.sum_apply]
  unfold X'

  /- simp? [ algebra_compatible_smul R, Algebra.algebraMap_self,
    RingHomCompTriple.comp_apply, map_smul, Basis.repr_self, Finsupp.coe_smul, Pi.smul_apply]
   -/
  rw [Finsupp.sum_eq_single d]
  · simp only [Finsupp.mapRange_apply, ite_smul]
    --simp only [Finsupp.mapRange_apply, Finsupp.single_eq_same,X']
    split_ifs with hd
    · simp [hd]
      have : ((basis R M b).repr ((0 : MvPolynomial R ℤ) • (basis R M b) d)) = 0 := by
        simp only [EmbeddingLike.map_eq_zero_iff]
        /- have := algebra_compatible_smul (R := R) (A := MvPolynomial R ℤ)
           (M := DividedPowerAlgebra R M) (m := basis R M b d) (r := 0)
         -/
        have : (0 : MvPolynomial R ℤ) = algebraMap ℤ (MvPolynomial R ℤ) 0 := by
          simp only [MvPolynomial.algebraMap_eq, eq_intCast, Int.cast_zero]

        rw [this]
        letI : IsScalarTower ℤ (MvPolynomial R ℤ) (DividedPowerAlgebra R M) := by
          unfold DividedPowerAlgebra
          exact RingQuot.instIsScalarTower (Rel R M)
        have := algebra_compatible_smul (R := ℤ) (A := MvPolynomial R ℤ) (r := 0) (m := basis R M b d)
        sorry
      simp [this]
